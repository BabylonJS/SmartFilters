...
const shaderProgram = injectDisableUniform({
    fragment: {
        ...
        const: `
            const vec3 _luminanceVector_ = vec3(0.2126, 0.7152, 0.0722);
            `,

        mainFunctionName: "_twoTone_",
        ...
        functions: [
            {
                name: "_twoTone_",
                code: `
                vec4 _twoTone_(vec2 vUV) {
                    vec3 color = texture2D(_input_, vUV).rgb;
                    float luma = clamp(dot(color, _luminanceVector_), 0.5, 1.);
                    vec3 bg = mix(_colorA_.rgb, _colorB_.rgb, luma);
                    return vec4(bg, 1.0);
                }
            `,
            },
...
/**
 * The shader bindings for the TwoToneBlock.
 */
export class TwoToneBinding extends ShaderBinding {
...
/**
 * Applies a two-tone effect to the input texture.
 */
export class TwoToneBlock extends ShaderBlock {
    /**
     * The class name of the block.
     */
    public static override ClassName = "TwoToneBlock";
    ...
    public getShaderBinding(): ShaderBinding {
        ...
        return new TwoToneBinding(this, input, colorA, colorB);
    }
}
