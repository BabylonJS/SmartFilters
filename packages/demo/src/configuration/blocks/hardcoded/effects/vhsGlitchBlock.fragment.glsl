...
vec4 mainImage(vec2 vUV) { // main
  ...
  vec4 fragColor = texture(input, vec2(xpos, uv.y));
  ...
  fragColor.g = mix(fragColor.r, texture(input, vec2(xpos + noise * 0.05, uv.y)).g, 0.25);
  fragColor.b = mix(fragColor.r, texture(input, vec2(xpos - noise * 0.05, uv.y)).b, 0.25);
