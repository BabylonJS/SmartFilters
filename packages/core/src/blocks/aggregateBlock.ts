...
export abstract class AggregateBlock extends BaseBlock {
     ...
     */
    public _mergeIntoSmartFilter(
        mergedAggregateBlocks: AggregateBlock[]
    ): void {
        // Rewire output connections
        for (const [internalConnectionPoint, externalConnectionPoint] of this
            ._aggregatedOutputs) {
            const endpointsToConnectTo =
                externalConnectionPoint.endpoints.slice();
        ...
        for (const [internalConnectionPoint, externalConnectionPoint] of this
            ._aggregatedInputs) {
            const connectedToExternalConnectionPoint =
                externalConnectionPoint.connectedTo;
            if (connectedToExternalConnectionPoint) {
                connectedToExternalConnectionPoint.disconnectFrom(
                    externalConnectionPoint
                );
                connectedToExternalConnectionPoint.connectTo(
        ...
        for (const aggregateOutput of this._aggregatedOutputs) {
            ...
            internalConnectionPoint.ownerBlock.visit(
                {},
                (block: BaseBlock, _extraData: Object) => {
    ...
    public _unmergeFromSmartFilter(): void {
        for (const [internalConnectionPoint, externalConnectionPoint] of this
            ._aggregatedOutputs) {
            const endpointsToConnectTo =
                internalConnectionPoint.endpoints.slice();
        ...
        for (const [internalConnectionPoint, externalConnectionPoint] of this
            ._aggregatedInputs) {
            const connectedToInternalConnectionPoint =
                internalConnectionPoint.connectedTo;
            if (connectedToInternalConnectionPoint) {
                connectedToInternalConnectionPoint.disconnectFrom(
                    internalConnectionPoint
                );
                connectedToInternalConnectionPoint.connectTo(
                    externalConnectionPoint
                );
            }
        }
    ...
    protected _registerSubfilterInput<U extends ConnectionPointType>(
        ...
        const externalInputConnectionPoint = this._registerInput(
            name,
            internalConnectionPoint.type
        );

        this._aggregatedInputs.push([
            internalConnectionPoint,
            externalInputConnectionPoint,
        ]);
    ...
    protected _registerSubfilterOutput<U extends ConnectionPointType>(
        ...
        const externalOutputConnectionPoint = this._registerOutput(
            name,
            internalConnectionPoint.type
        );

        this._aggregatedOutputs.push([
            internalConnectionPoint,
            externalOutputConnectionPoint,
        ]);
